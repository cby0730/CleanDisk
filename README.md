...
### 🤖 AI 智慧分析
- **智慧建議**：使用本地 LLM 模型分析檔案，提供保留或刪除建議
- **隱私優先**：所有 AI 運算皆在本地執行，檔案資訊絕不上傳雲端
- **多模型支援**：支援多種不同大小和速度的模型（如 Qwen, Llama, DeepSeek）
- **詳細理由**：AI 會提供判斷的信心度以及具體的建議理由
...
- **流暢的進度顯示**：智慧節流機制，避免過度頻繁的 UI 更新
- **清晰的載入回饋**：精確的模型下載/載入狀態提示，不會錯過任何訊息

## 🖥️ 系統需求

- **作業系統**：macOS 15.5 (Sequoia) 或更新版本
- **架構**：支援 Apple Silicon (M1/M2/M3/M4) Mac
- **記憶體**：建議 16GB RAM 以上（若要使用較大的 AI 模型）
- **磁碟空間**：約 50MB 安裝空間（AI 模型需額外下載，約 1.5GB - 12GB 不等）
